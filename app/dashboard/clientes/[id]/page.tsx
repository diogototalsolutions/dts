...
export default async function ClienteDetalhePage({ params }: { params: { id: string } }) {
  const client = await prisma.client.findUnique({ where: { id: params.id }, include: { services: true } });
  if (!client) notFound();
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded bg-white p-4 shadow-sm">
        ...
        {client.services.length === 0 ? <p>Sem serviços associados.</p> : (
          <ul className="space-y-2">
            {client.services.map((service) => <li key={service.id}>{service.titulo} — {new Date(service.data).toLocaleDateString('pt-PT')}</li>)}
          </ul>
        )}
      </div>
    </div>
